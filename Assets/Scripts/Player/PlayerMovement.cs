...
//        // Notify the current mouse (if any) to flip
//        if (currentMouse != null)
//        {
//            currentMouse.FlipToMatchPlayer();
//        }
//    }
...
//        if (currentMouse != null)
//        {
//            // Drop the previous mouse at the current pickup position
//            currentMouse.DropMouse(newMouse.transform.position);
//        }
...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (currentMouse != null && _isGrounded)
            {
                Collider2D nearbyMouse = Physics2D.OverlapCircle(transform.position, 1f, LayerMask.GetMask("Mice"));

                if (nearbyMouse != null)
                {
                    // Pick up the nearby mouse (swap)
                    Mice newMouse = nearbyMouse.GetComponent<Mice>();
                    PickupMouse(newMouse);
                }
                else
                {
                    // No nearby mouse, just drop the current one
                    DropCurrentMouse();
                }
            }
        }
    ...
    {
        return _isGrounded; // Return the existing grounded status
    }
    ...
    public void PickupMouse(Mice newMouse)
    {
        if (currentMouse != null)
        {
            // Drop the existing mouse before picking up a new one
            DropCurrentMouse();
        }
        ...
        if (currentMouse != null)
        {
            ...
            ToggleCorrespondingPlatforms(currentMouse.isBlueMouse); // Activate new platforms
        }
    }

    private void DropCurrentMouse()
    {
        if (currentMouse != null)
        {
            // Drop the mouse at the designated drop position
            Vector3 dropPosition = dropPoint.position;
            Debug.Log("Attempting to Drop Mouse at: " + dropPosition);
            currentMouse.DropMouse(dropPosition);

            // Deactivate corresponding platforms
            ToggleCorrespondingPlatforms(!currentMouse.isBlueMouse);
