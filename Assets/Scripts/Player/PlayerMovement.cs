...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void PickupMouse(Mice newMouse)
    {
        ...
        {
            StopCoroutine(pickupCoroutine); // Cancel any previous pickup
        }
        ...
        pickupCoroutine = StartCoroutine(PlayPickupSequence(newMouse));
    }
    ...
    private IEnumerator PlayPickupSequence(Mice newMouse)
    {
        ...
        if (!IsAlive)
        {
            Debug.Log("[Pickup] Interrupted by player death.");
    ...
    public void OnHit(int damage, Vector2 knockback)
    {
        ...
        // Cancel pickup animation if in progress
        if (pickupCoroutine != null)
        {
            Debug.Log("[OnHit] Cancelling mouse pickup due to hit.");
        ...
        StopAllCoroutines(); // Prevent stacking knockbacks or leftover state
